...
# d <- ggplot(all_data, aes(mean_lon, mean_lat)) +
#         borders(regions="poland", size=1, fill="#D3D3D3") +
#         coord_fixed(1.6) +
#         geom_density(aes(color = city, size = city_count), alpha=0.5) +
#         labs(color = "miasto", size = "liczba zapytań") +
#         scale_size_continuous(range = c(1, 30)) +
#         scale_color_manual(values=viri) +
#         theme_void(base_size=20) +
...
p <- ggplot(all_data, aes(x = date)) +
        ...
        ggtitle("ilość danych w kazdej kategorii w danym okresie")
...
year_id <- 2019
  year_data <- all_data[all_data$year == year_id, ]
  year_isp <- data.frame(table(year_data$isp))
  year_isp <- dplyr::arrange(year_isp, desc(Freq))
  year_isp$Var1 <- as.character(year_isp$Var1)
  year_isp <- dplyr::inner_join(year_isp, year_data[, c("isp", "user_category")], by = c("Var1" = "isp"))
  year_isp <- year_isp[!duplicated(year_isp), ]
  year_isp <- year_isp[1:10, c(1, 3, 2)]
  colnames(year_isp) <- c("nazwa użytkownika", "kategoria", "ilość pobrań")
  row.names(year_isp) <- NULL
  
  knitr::kable(year_isp, caption = paste0("Rok ", year_id)) %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                 position = "float_left", )
